...
server.post("/resize-video", async (request, reply) => {
  ...
  const tempInput = path.join(STORAGE_ROOT, `temp_in_${tempId}.mp4`);
  const tempOutput = path.join(STORAGE_ROOT, `temp_out_${tempId}.mp4`);

  try {
    // Save input to temp file
    await pipeline(filePart.file, createWriteStream(tempInput));
    ...
    let probeData = "";
    for await (const chunk of probe.stdout) probeData += chunk;
    const [width, height] = probeData.trim().split("x").map(Number);
    ...
    if (width <= 1080 && height <= 1080) {
      ...
      const stream = createReadStream(tempInput);
      reply.header("Content-Type", "video/mp4");
      reply.header("Content-Length", stats.size);
      // Clean up input after sending (tricky with stream, so we might need to wait or rely on OS cleanup/cron, 
      // but for now let's just send it. Actually, we should probably delete it after.
      // A better way for this simple server: read to buffer if small, or just leave it?
      // Let's just return the stream and delete on finish.
      return reply.send(stream);
    }

    // Resize needed
    // scale='min(1080,iw):min(1080,ih):force_original_aspect_ratio=decrease'
    // Actually, "at least one side <= 1080" means if BOTH are > 1080, we need to downscale until ONE is 1080.
    // But usually "max 1080p" means fit within 1080x1080 bounding box, OR it means the short side is 1080?
    // User said: "atleast one side having side of 1080 px or less".
    // So if we have 1920x2000, we need to resize.
    // If we have 1920x1080, it is fine.
    // If we have 1080x1920, it is fine.
    // So we only resize if width > 1080 AND height > 1080.

    let scaleFilter = "";
    if (width > 1080 && height > 1080) {
      // Downscale so the smaller dimension becomes 1080 (or larger dimension? usually we want to fit IN 1080)
      // User said: "downscales the video proportionally to just fit within 1080 px length"
      // This usually means the long side should be 1080. Let's assume fit within 1080x1080 box to be safe.
      scaleFilter = "scale='min(1080,iw)':'min(1080,ih)':force_original_aspect_ratio=decrease";
    } else {
      // Should not happen given the if check, but just in case
