...
server.post("/files", async (request, reply) => {
  ...
  for await (const part of parts) {
    if (part.type === "file" && part.fieldname === "file") {
      filePart = part;
    } else if (part.type === "field" && part.fieldname === "workspace") {
  ...
  if (!relPath) {
    reply.code(400).send({ error: "Missing target path" });
    return;
  }
  ...
  try {
    ...
    reply.code(500).send({ error: error.message ?? "Failed to save file" });
  }
});
